...
def window_partition(x, window_size):
    ...
    """
    B, H, W, C = x.shape
    x = rearrange(x, "b (h win1) (w win2) c -> b h win1 w win2 c", win1=window_size, win2=window_size)
...
class WindowAttention(nn.Layer):
    ...
    def forward(self, x, mask=None):
        ...
        relative_position_index = self.relative_position_index.reshape([-1]).numpy()

        relative_position_bias = paddle.to_tensor(relative_position_bias_table[relative_position_index])
...
class BasicLayer(nn.Layer):
    ...
    def forward(self, x, H, W):
        ...
        Hp = int(paddle.ceil(paddle.to_tensor([H / self.window_size]))) * self.window_size
        Wp = int(paddle.ceil(paddle.to_tensor([W / self.window_size]))) * self.window_size
...
class SwinTransformer(nn.Layer):
    ...
    def __init__(self,
        ...
        if self.ape:
            ...
            self.absolute_pos_embed = self.create_parameter([1, embed_dim, patches_resolution[0], patches_resolution[1]], default_initializer=nn.initializer.TruncatedNormal(std=.02))

        self.pos_drop = nn.Dropout(p=drop_rate)

        # stochastic depth
        dpr = [float(x) for x in paddle.linspace(0, drop_path_rate, sum(depths))]  # stochastic depth decay rule
    ...
    def forward(self, x): # B C Wh Ww
        ...
        if self.ape:
            ...
            x = (x + absolute_pos_embed).flatten(2) # B C Wh Ww -> B C wh, wh=Wh*Ww
