...
class WindowAttention(nn.Layer):
    def __init__(self, dim, heads, head_dim, shifted, window_size, relative_pos_embedding):
        ...
        if self.shifted:
            ...
            self.upper_lower_mask = self.create_parameter([window_size ** 2, window_size ** 2], dtype='float32')
            self.upper_lower_mask.set_value(create_mask(window_size=window_size, displacement=displacement,
                                                             upper_lower=True, left_right=False))
            self.left_right_mask = self.create_parameter([window_size ** 2, window_size ** 2], dtype='float32')
            self.left_right_mask.set_value(create_mask(window_size=window_size, displacement=displacement,
        ...
        if self.relative_pos_embedding:
            ...
            self.pos_embedding = self.create_parameter(shape=[2 * window_size - 1, 2 * window_size - 1], dtype='float32')
        else:
            self.pos_embedding = self.create_parameter(shape=[window_size ** 2, window_size ** 2], dtype='float32')
