...
class AxialAttention(nn.Layer):
    def __init__(self, in_planes, out_planes, groups=8, kernel_size=56,
        ...
        # Position embedding
        self.relative = nn.Parameter(paddle.randn(self.group_planes * 2, kernel_size * 2 - 1), requires_grad=True)
        query_index = paddle.arange(kernel_size).unsqueeze(0)
        key_index = paddle.arange(kernel_size).unsqueeze(1)
    ...
    def forward(self, x):
        ...
        if self.width:
            ...
            x = x.tranpose([0, 3, 1, 2])  # N, W, C, H
        N, W, C, H = x.shape
        x = x.reshape([N * W, C, H])

        # Transformations
        qkv = self.bn_qkv(self.qkv_transform(x))
        q, k, v = paddle.split(qkv.reshape(N * W, self.groups, self.group_planes * 2, H), [self.group_planes // 2, self.group_planes // 2, self.group_planes], axis=2)

        # Calculate position embedding
        all_embeddings = paddle.index_select(self.relative, 1, self.flatten_index).view(self.group_planes * 2, self.kernel_size, self.kernel_size)
        q_embedding, k_embedding, v_embedding = paddle.split(all_embeddings, [self.group_planes // 2, self.group_planes // 2, self.group_planes], axis=0)
        
        qr = paddle.einsum('bgci,cij->bgij', q, q_embedding)
        kr = paddle.einsum('bgci,cij->bgij', k, k_embedding).transpose(2, 3)
        
        qk = paddle.einsum('bgci, bgcj->bgij', q, k)
        
        stacked_similarity = paddle.concat([qk, qr, kr], axis=1)
        stacked_similarity = self.bn_similarity(stacked_similarity).view(N * W, 3, self.groups, H, H).sum(axis=1)
        #stacked_similarity = self.bn_qr(qr) + self.bn_kr(kr) + self.bn_qk(qk)
        # (N, groups, H, H, W)
        similarity = F.softmax(stacked_similarity, axis=3)
        sv = paddle.einsum('bgij,bgcj->bgci', similarity, v)
        sve = paddle.einsum('bgij,cij->bgci', similarity, v_embedding)
        stacked_output = paddle.concat([sv, sve], axis=-1).view(N * W, self.out_planes * 2, H)
        output = self.bn_output(stacked_output).view(N, W, self.out_planes, 2, H).sum(axis=-2)
...
class AxialAttention_dynamic(nn.Layer):
    def __init__(self, in_planes, out_planes, groups=8, kernel_size=56,
                 stride=1, bias=False, width=False):
        assert (in_planes % groups == 0) and (out_planes % groups == 0)
        ...
        self.f_qr = nn.Parameter(paddle.tensor(0.1),  requires_grad=False) 
        self.f_kr = nn.Parameter(paddle.tensor(0.1),  requires_grad=False)
        self.f_sve = nn.Parameter(paddle.tensor(0.1),  requires_grad=False)
        self.f_sv = nn.Parameter(paddle.tensor(1.0),  requires_grad=False)
    ...
    def forward(self, x):
        ...
        # multiply by factors
        qr = paddle.mul(qr, self.f_qr)
        kr = paddle.mul(kr, self.f_kr)
        ...
        # multiply by factors
        sv = paddle.mul(sv, self.f_sv)
        sve = paddle.mul(sve, self.f_sve)
...
class AxialAttention_wopos(nn.Layer):
    def __init__(self, in_planes, out_planes, groups=8, kernel_size=56,
                 stride=1, bias=False, width=False):
        assert (in_planes % groups == 0) and (out_planes % groups == 0)
    ...
    def forward(self, x):
        ...
        stacked_similarity = self.bn_similarity(qk).reshape(N * W, 1, self.groups, H, H).sum(axis=1).contiguous()

        similarity = F.softmax(stacked_similarity, axis=3)
        sv = paddle.einsum('bgij,bgcj->bgci', similarity, v)

        sv = sv.reshape(N*W,self.out_planes * 1, H).contiguous()
        output = self.bn_output(sv).reshape(N, W, self.out_planes, 1, H).sum(axis=-2).contiguous()
...
class AxialBlock_dynamic(nn.Layer):
    expansion = 2
...
class AxialBlock_wopos(nn.Layer):
    expansion = 2
...
class ResAxialAttentionUNet(nn.Layer):
